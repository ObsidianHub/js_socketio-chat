...
io.on('connection', (socket) => {
  ...
  socket.on('new user', (name) => {
    ...
    socket.broadcast
      .to(socket.room)
      .emit('new user joined', socket.username); // to everyone in the room except the user
  ...
  socket.on('roomchange', (index) => {
    socket.broadcast
      .to(socket.room)
      .emit('has left the room', socket.username);
    socket.broadcast
      .to(socket.room)
      .emit('userswitchedroom', {
        usernames,
    ...
    socket.broadcast
      .to(socket.room)
      .emit('new user joined', socket.username);
    io.emit('roommates', { usernames, room: socket.room });
  });
});
